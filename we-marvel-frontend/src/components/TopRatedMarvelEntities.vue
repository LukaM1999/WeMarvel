    ...
    <div class="col">
      <ejs-grid :key="tableKey" ref="grid" :dataSource="entities" :allowPaging="true"
        ...
        <e-columns>
          ...
          <e-column field='ratingCount' headerText='Raters' textAlign="Center" width=40></e-column>
        </e-columns>
        <template v-slot:rowTemplate="{data}">
          <tr>
            ...
            <td>
              <span class="avg-rating">{{data.ratingCount}}</span>
            </td>
          </tr>
        </template>
      </ejs-grid>
...
export default {
  ...
  methods: {
    async getCharacters(){
      const response = await axios.get(`${process.env.VUE_APP_BACKEND}/${this.type}/topRated`);
