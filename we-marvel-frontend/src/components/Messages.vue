  ...
  <div class="window-container" :class="{ 'window-mobile': isDevice }">
    <div v-if="addNewRoom" class="row mt-3 mb-3 justify-content-center">
      <div class="col-8">
        <div class="row">
          <div class="col">
            <Users @user-selected="userSelected" :username="currentUserProp.username"/>
          </div>
        </div>
        <div class="row mt-2">
          <div class="col d-flex justify-content-end">
            <ejs-button content="Create chat" isPrimary="true" :disabled="!newChatUser" @click="createRoom">
            </ejs-button>
          </div>
          <div class="col d-flex">
            <ejs-button content="Cancel" @click="newChatCancelled">
            </ejs-button>
          </div>
        </div>
      </div>
...
<script>
import * as firestoreService from '../firebaseServices/firestore'
import * as firebaseService from '../firebaseServices/firebase'
import * as storageService from '../firebaseServices/storage'
...
export default {
  ...
  async mounted() {
    this.currentUserId = this.currentUserProp.id.toString();
    this.currentUser = this.currentUserProp;
  ...
  methods: {
    async getFriends(){
      await axios.get(`${process.env.VUE_APP_BACKEND}/friend/${auth.currentUser.displayName}/accepted`)
          .then(({data}) => {
            this.friends = data?.flatMap(c => {
              if(c.senderId === this.currentUserId){
                return {
                  _id: c.receiverId.toString(),
                  username: c.receiverUsername,
                  profilePicture: c.receiverImageUrl
                }
              }
              return {
                _id: c.senderId.toString(),
                username: c.senderUsername,
                profilePicture: c.senderImageUrl
              }
            })
          })
    },
    ...
    async fetchMoreRooms() {
      ...
      Object.keys(roomList).forEach(key => {
        ...
        const roomAvatar = roomContacts[0]._id === this.currentUserId ?
            roomContacts[1].profilePicture : roomContacts[0].profilePicture;

        formattedRooms.push({
          ...room,
          roomId: key,
          avatar: roomAvatar || '/placeholder.jpg',
    ...
    async sendNotification(content, roomId) {
      ...
      await axios.post(`${process.env.VUE_APP_BACKEND}/notification/message`, {
        type: 'new_message',
    ...
    async createRoom() {
      ...
      await firestoreService.addIdentifiedUser(this.currentUserId, {
        ...
        profilePicture: this.currentUser.imageUrl,
      })
