        ...
        <td>
          <div class="row">
            <div class="col">
              <div class="row mb-2">
                <div class="col">
                  <b style="font-size: 16px;"><a class="custom-link" :href="`/forum/board/1/character/${data.marvelEntityId}/topic/${data.id}`"
                                                 @click.prevent="openTopic(data.marvelEntityId, data.id)">{{ data.title }}</a></b>
                </div>
              </div>
              <div class="row">
                <div class="col">
                  <a class="custom-link" :href="`/character/${data.marvelEntityId}`"
                     @click.prevent="openCharacter(data.marvelEntityId)">
...
export default {
  ...
  methods: {
    ...
    openTopic(characterId, topicId){
      router.push(`/forum/board/1/character/${characterId}/topic/${topicId}`);
    },
    openCharacter(id){
      router.push(`/forum/board/1/character/${id}`);
    },
